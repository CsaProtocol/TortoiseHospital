...
CREATE DOMAIN Profile AS CHAR(1)
    CHECK(VALUE='V' OR VALUE='R' OR VALUE='O' OR VALUE='L' OR VALUE='A');
...
BEGIN
    ...
    IF((SELECT employee_ID FROM employee WHERE employee_ID LIKE new.profiletype || '%' ORDER BY employee_ID DESC LIMIT 1) IS NULL) THEN
        ...
        CASE new.profiletype
            ...
            WHEN 'A' THEN emp_id := 'ADMIN' || lpad('1', 5, '0');
        END CASE;
    ...
    IF(NEW.profiletype = 'A') THEN
        RAISE EXCEPTION 'One admin allowed';
    END IF;
...
CREATE TABLE tank (
    ...
    surrogate_tank INT4 PRIMARY KEY,*/ --DELETED

    -- Vincolo per garantire l'unicità della combinazione di tank_ID e center_ID
    -- DELETED -- UNIQUE (tank_ID, center_ID),
    PRIMARY KEY(tank_ID, center_ID),
...
/* DELETED  -- CREATE FUNCTION tankUIDGeneration()
RETURNS TRIGGER AS $$
DECLARE
    counter INT;
...
CREATE TABLE turtle (
    ...
    FOREIGN KEY (tank_ID, center_ID) REFERENCES tank(tank_ID, center_ID),
...
BEGIN
    ...
    turtleCount := (SELECT COUNT(*) FROM turtle WHERE center_ID = NEW.center_ID AND tank_ID = NEW.tank_ID);

    -- Verifica se il numero di tartarughe supera la capacità massima della vasca
    IF turtleCount > (SELECT capacity FROM tank WHERE center_ID = NEW.center_ID AND tank_ID = NEW.tank_ID) THEN
...
BEGIN
    ...
    UPDATE turtle
    SET center_ID = 'NULL', tank_ID = 'NULL'
...
RETURNS TRIGGER AS $$
BEGIN
	-- Verifica se la data corrente è successiva alla data di inserimento dell'esame
	IF CURRENT_DATE > OLD.ex_date
...
-- INSERT INTO Measurement(turtle_ID, width, length, weight, m